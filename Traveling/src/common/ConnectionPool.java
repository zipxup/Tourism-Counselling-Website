...
public class ConnectionPool {
	...
	private String dbUrl = "jdbc:mysql://localhost:3306/traveling"; // 数据 URL

	private String dbUsername = "localhost"; // 数据库用户名

	private String dbPassword = "123456"; // 数据库用户密码

	private String testTable = "announcement"; // 测试连接是否可用的测试表名，默认没有测试表

	private int initialConnections = 80; // 连接池的初始大小

	private int incrementalConnections = 5;// 连接池自动增加的大小

	private int maxConnections = 500; // 连接池最大的大小

	private Vector<PooledConnection> connectionVector = null; // 存放连接池中数据库连接的向量
																// , 初始时为 null
	...
	/**
	 * 创建由 numConnections 指定数目的数据库连接 , 并把这些连接 放入 connectionVector 向量中
	 * 
	...
	private void createConnections(int numConnections) throws SQLException {
		...
		for (int i = 0; i < numConnections; i++) {
			...
			if (this.maxConnections > 0
					&& this.connectionVector.size() >= this.maxConnections) {
				break;
			}

			// 增加一个连接到连接池中（向量 connectionVector 中）
			try {
				connectionVector.addElement(new PooledConnection(
						createConnection()));
				s++;
				System.out.println("增加一个连接到连接池中,已增加" + s + "个");
	...
	private Connection getFreeConnection() throws SQLException {
		// 从连接池中获得一个可用的数据库连接
		i++;
		System.out.println("获得一个数据库可用连接,连接已使用" + i + "次!!!");
			...
			if (!pConn.isBusy()) {
				...
				if (!testConnection(conn)) {
					...
					try {
						...
						System.out.println(" 查找一个可用的数据库连接,创建数据库连接失败！ "
								+ e.getMessage());
						return null;
					}
	...
	public synchronized void refreshConnections() throws SQLException {
		...
		while (enumerate.hasMoreElements()) {
			...
			pConn.setConnection(createConnection());
			pConn.setBusy(false);
		}
	}
